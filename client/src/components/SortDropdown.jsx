  ...
  return (
    <div className="relative z-20">
      <button
        ...
        className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-4 py-2 rounded-lg text-xs font-bold text-slate-700 dark:text-slate-300 hover:border-emerald-500 transition-colors shadow-sm"
      >
          ...
          {options.map((option) => (
            <button
              ...
              className={`w-full text-left px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${
                sort === option.value ? 'font-bold text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'text-slate-700 dark:text-slate-300 font-medium'
              }`}
            >
              {option.label}
            </button>
