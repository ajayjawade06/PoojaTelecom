...
const HeroCarousel = () => {
 ...
 >
 <div className="absolute inset-0 bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-blue-950/20"></div>
 
 <div className="main-container h-full flex flex-col md:flex-row items-center justify-between relative z-10 py-8">
 <div className="max-w-lg text-center md:text-left transition-all duration-200 delay-200">
 {slide.subtitle && (
 <span className="inline-flex items-center gap-1.5 py-1 px-3 rounded-md bg-blue-500/10 text-blue-600 dark:text-blue-400 font-bold text-[9px] uppercase tracking-widest mb-4 border border-blue-500/20">
 ...
 <Link to={slide.link || '/search/all'} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold py-2.5 px-6 rounded-lg transition-all text-[12px] flex items-center gap-2">
 ...
 className="h-full w-full object-contain drop-shadow-2xl transform transition-transform" 
 alt={slide.title} 
 />
 </div>
 </div>
 ...
 className={`h-1 rounded-full transition-all ${i === current ? 'w-8 bg-blue-500' : 'w-2 bg-slate-300 dark:bg-slate-700'}`}
 ></button>
 ))}
 </div>
 )}
