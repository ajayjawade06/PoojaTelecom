...
const UserList = () => {
  ...
  const deleteHandler = async (id) => {
    if (window.confirm('WARNING: Initiate permanent agent termination? Protocol is irreversible.')) {
      try {
        await deleteUser(id);
        refetch();
      } catch (err) {
    ...
    <div className="container mx-auto px-4 py-12 max-w-7xl animate-fade-in relative z-10 w-full flex flex-col items-center">
      <div className="w-full relative z-10">
        <div className="flex items-center gap-4 mb-10 relative z-10">
          ...
          <div>
             <h1 className="text-4xl font-black text-white tracking-tighter">Operative Roster</h1>
             <p className="text-slate-400 font-medium mt-1">Directory of all authorized network identities.</p>
          </div>
        </div>
            ...
            <div className="overflow-x-auto rounded-[2rem] bg-slate-950 border border-white/5 relative z-10">
              <table className="w-full text-left text-sm whitespace-nowrap border-collapse">
                <thead className="bg-white/5 border-b border-white/10 uppercase tracking-widest text-[10px] font-black text-slate-400">
                  <tr>
                    <th className="px-6 py-6 border-r border-white/5">Identifier Node</th>
                    <th className="px-6 py-6">Designation</th>
                    <th className="px-6 py-6">Comms Link</th>
                    <th className="px-6 py-6 text-center">Clearance Status</th>
                    <th className="px-6 py-6 border-l border-white/5 text-center">Directives</th>
                  </tr>
                </thead>
                      ...
                      <td className="px-6 py-5 text-center">
                        {user.isAdmin ? (
                          <div className="inline-flex items-center gap-2 bg-emerald-500/10 px-3 py-1.5 rounded-xl border border-emerald-500/20 shadow-[0_0_10px_rgba(16,185,129,0.1)]">
                             ...
                             <span className="text-[10px] font-black uppercase text-emerald-400 tracking-widest">Admin Authorization</span>
                          </div>
                        ) : (
                          <div className="inline-flex items-center gap-2 bg-slate-800 px-3 py-1.5 rounded-xl border border-white/5">
                             ...
                             <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Standard Operative</span>
                          </div>
                        )}
                      </td>
