...
const ProductEdit = () => {
 ...
 const uploadAdditionalFileHandler = async (e) => {
 const formData = new FormData();
 formData.append('image', e.target.files[0]);
 try {
 const res = await uploadProductImage(formData).unwrap();
 ...
 <Link to="/admin/productlist" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-500 mb-8 transition-colors">
 ...
 <div className="flex items-center justify-between bg-blue-500/5 border border-blue-500/10 p-4 rounded-xl mb-6">
 <div>
 <h3 className="text-[11px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">Visibility Status</h3>
 ...
 <input 
 type="text" 
 className="flex-grow bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 py-3 px-4 rounded-xl text-xs font-bold outline-none focus:border-blue-500/30 dark:text-white"
 value={image}
 onChange={e => setImage(e.target.value)}
 />
 <label className="cursor-pointer bg-slate-100 dark:bg-white/10 px-4 flex items-center justify-center rounded-xl hover:bg-blue-500 hover:text-white transition-all text-slate-500">
 ...
 {loadingUpload && <p className="text-[9px] font-black text-blue-500 uppercase tracking-widest animate-pulse mt-1">Uploading...</p>}
 </div>

 <div className="space-y-1.5 mt-4">
 <div className="flex items-center justify-between">
 <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Additional Images ({images.length}/10)</label>
 {images.length < 10 && (
 <label className="cursor-pointer bg-slate-100 dark:bg-white/10 px-3 py-1 flex items-center justify-center rounded-lg hover:bg-blue-500 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest text-slate-500">
 <FaCloudUploadAlt size={12} className="mr-1" /> Add
 <input type="file" className="hidden" onChange={uploadAdditionalFileHandler} />
 </label>
 )}
 </div>
 {images.length > 0 && (
 ...
 <div key={index} className="relative w-20 h-20 bg-white dark:bg-slate-950 border border-slate-100 dark:border-white/10 rounded-xl p-2 shrink-0 group">
 <img src={getFullImageUrl(img)} alt={`additional-${index}`} className="w-full h-full object-contain" />
 <button 
 type="button" 
 onClick={() => removeAdditionalImage(index)}
 className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-1 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity z-10"
 >
 <FaTimes size={10} />
 </button>
 </div>
 ...
 <textarea 
 rows="3"
 className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 py-3 px-4 rounded-xl text-xs font-bold outline-none focus:border-blue-500/30 dark:text-white resize-none"
 value={description}
 onChange={e => setDescription(e.target.value)}
 />
 </div>

 <button type="submit" className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 h-12 rounded-xl font-black text-[11px] uppercase tracking-widest shadow-xl transition-all flex items-center justify-center gap-3 mt-4">
