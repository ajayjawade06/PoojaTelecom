...
const ShippingPage = () => {
  ...
  return (
    <FormContainer>
      ...
      <h1 className="text-4xl font-black mb-8 text-white tracking-tight flex items-center gap-3">
        <FaMapMarkerAlt className="text-emerald-500" /> Logistics Hub
      </h1>

      {/* Saved Addresses feature */}
      {userInfo && userInfo.addresses && userInfo.addresses.length > 0 && (
        <div className="mb-10">
           <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2"><FaHistory className="text-slate-500" /> Access Saved Coordinates</h3>
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             {userInfo.addresses.map((addr, idx) => (
               <div 
                 key={idx} 
                 onClick={() => autofillSavedAddress(addr)}
                 className="p-5 border border-white/5 bg-white/5 rounded-2xl cursor-pointer hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all relative group shadow-xl shadow-black/20"
               >
                 <div className="font-black text-white text-sm mb-1 group-hover:text-emerald-400 transition-colors truncate">{addr.address}</div>
                 <div className="text-xs text-slate-400 font-medium">{addr.city}, {addr.postalCode}</div>
                 <div className="text-xs text-slate-500 mt-1 uppercase tracking-widest">{addr.country}</div>
      ...
      >
         <FaMapMarkerAlt size={14} /> {geoLoading ? 'Acquiring Satellites...' : 'Auto-Detect Coordinates'}
      </button>

      <div className="relative flex py-5 items-center mb-6">
         <div className="flex-grow border-t border-white/5"></div>
         <span className="flex-shrink-0 mx-4 text-slate-500 text-[10px] font-black uppercase tracking-widest">Manual Entry Override</span>
         <div className="flex-grow border-t border-white/5"></div>
      </div>
      
      <form onSubmit={submitHandler} className="space-y-6">
        <div className="group">
          <label className="block text-slate-400 text-xs font-black uppercase tracking-widest mb-2 transition-colors group-focus-within:text-emerald-400">Street Vector</label>
          <input
            type="text"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="group">
            <label className="block text-slate-400 text-xs font-black uppercase tracking-widest mb-2 transition-colors group-focus-within:text-emerald-400">Sector / City</label>
            <input
              type="text"
          ...
          <div className="group">
            <label className="block text-slate-400 text-xs font-black uppercase tracking-widest mb-2 transition-colors group-focus-within:text-emerald-400">Zip / Node Code</label>
            <input
              type="text"
              placeholder="Z-4004"
              className="w-full px-5 py-4 rounded-2xl border border-white/10 bg-white/5 focus:bg-white/10 focus:border-emerald-500/50 text-white font-medium transition-all outline-none placeholder-slate-600 ring-4 ring-transparent focus:ring-emerald-500/10"
        ...
        <div className="group pb-4">
          <label className="block text-slate-400 text-xs font-black uppercase tracking-widest mb-2 transition-colors group-focus-within:text-emerald-400">Territory / Country</label>
          <input
            type="text"
            placeholder="Earth"
            className="w-full px-5 py-4 rounded-2xl border border-white/10 bg-white/5 focus:bg-white/10 focus:border-emerald-500/50 text-white font-medium transition-all outline-none placeholder-slate-600 ring-4 ring-transparent focus:ring-emerald-500/10"
        ...
        >
          ...
          Set Coordinates <FaArrowRight className="group-hover:translate-x-1 transition-transform" />
        </button>
      </form>
    </FormContainer>
  );
